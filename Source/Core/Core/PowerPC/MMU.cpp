...
T MMU::ReadFromHardware(u32 em_address)
{
  ...
  const bool do_translate = translate_if == TranslateCondition::Always ||
                            (translate_if == TranslateCondition::MsrDrSet &&
...
MMU::TranslateAddressResult MMU::TranslatePageAddress(const EffectiveAddress address, bool* wi)
{
  ...
  for (int hash_func = 0; hash_func < 2; hash_func++)
  {
    ...
    for (int i = 0; i < 8; i++, pteg_addr += 8)
    {
      ...
      const u32 pteg = ReadFromHardware<pte_read_flag, u32, TranslateCondition::Never>(pteg_addr);

      if (pte1.Hex == pteg)
      {
        UPTE_Hi pte2(ReadFromHardware<pte_read_flag, u32, TranslateCondition::Never>(pteg_addr + 4));
